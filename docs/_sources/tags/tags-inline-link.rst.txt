...
The ``{@link}`` inline tag creates a link to the namepath or URL that
you specify. When you use the ``{@link}`` tag, you can also provide link
...
If you need to link to a tutorial, use the ```{@tutorial}`` inline
tag <tags-inline-tutorial.html>`__ instead of the ``{@link}`` tag.

Link formatting
---------------

By default, ``{@link}`` generates standard HTML anchor tags. However,
...
-  ``templates.monospaceLinks``: When set to ``true``, all links use a
   monospace font, except for links created with the ``{@linkplain}``
   tag.
