...
``@ignore`` tag has the following effects:

-  If you use the ``@ignore`` tag with the ``@class`` or ``@module``
   tag, the entire class or module will be omitted from the
   documentation.
-  If you use the ``@ignore`` tag with the ``@namespace`` tag, you must
   also add the ``@ignore`` tag to any child classes and namespaces.
...
``Jacket`` class. The ``@ignore`` tag must be added to both ``Clothes``
